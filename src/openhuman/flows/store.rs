...
/// lock-free.** [`init_schema`] stamps [`FLOWS_SCHEMA_VERSION`] only after a
...
/// far cheaper than the DDL batch it replaces. This matters more here than in the
/// sibling stores: `with_connection` (and so this check) runs once per node per
/// live run via `upsert_flow_run_step`, not just at store open.
///
...
/// returns (initialization must be atomic per path). Independent db paths contend
/// only during that rare init window, never on the hot path.
///
...
/// (`flow_definitions` present but missing a migrated column such as
/// `require_approval` / `graph_hash`, or one of the other tables) — falls through
/// to the idempotent [`init_schema`] and is re-migrated rather than trusted and
/// later failing with `no such table` / `no such column`. This validates the
/// whole schema, not just one table's presence, which the prior `sqlite_master`
/// probe could not.
///
...
fn ensure_schema_initialized(conn: &Connection, db_path: &Path) -> Result<()> {
    let is_current = || -> bool {
        conn.query_row("PRAGMA user_version", [], |row| row.get::<_, i64>(0))
            .unwrap_or(0)
            == FLOWS_SCHEMA_VERSION
    };

    // Lock-free fast path: an already-migrated database carries
    // FLOWS_SCHEMA_VERSION in its header, so the common case never acquires the
